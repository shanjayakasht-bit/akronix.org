    ...
    <div ref={ref} className="relative aspect-square lg:order-2">
      ...
      >
        ...
        <motion.div
          ...
          className="absolute inset-x-0 h-1/2 bg-gradient-to-b from-purple-400/0 via-purple-400/10
                     to-transparent opacity-0 group-hover:opacity-100"
        />
        <div className="absolute inset-0 bg-gradient-to-t from-black via-transparent/20 to-transparent" />
        <div className="absolute top-8 left-8 w-4 h-4 border-t-2 border-l-2 border-white/20 group-hover:border-purple-500/50 transition-colors" />
        <div className="absolute bottom-8 right-8 w-4 h-4 border-b-2 border-r-2 border-white/20 group-hover:border-purple-500/50 transition-colors" />
      </motion.div>
    </div>
  );
}
...
function WorkflowSection() {
  ...
  const nodes = useMemo(() => [
    { cx: 220, cy: 75, num: 1, label: "Sign-Up",          desc: "Trigger-Action logic: A new user sign-up initiates an automated chain reaction.", color: "#00F0FF", above: false },
    { cx: 450, cy: 25, num: 2, label: "Provision",        desc: "System instantly provisions a secure shard, workspace, and database for the user.", color: "#9D5BFF", above: true  },
    { cx: 670, cy: 75, num: 3, label: "Configure",        desc: "Automatically sets up the billing profile through Stripe/Paddle and dashboard.", color: "#00F0FF", above: false },
    { cx: 880, cy: 25, num: 4, label: "Deliver",          desc: "Product is delivered immediately, with multi-tenant resources managed automatically.", color: "#9D5BFF", above: true  },
  ], []);
  ...
  return (
    <section className="relative z-10 py-20">
      <div className="container-xl px-6">
        ...
        >
          <div className="h-px w-7 bg-purple-500" />
...
function BgOrbs() {
  ...
  return (
    <div ref={ref} className="absolute inset-0 pointer-events-none overflow-hidden z-0">
      <motion.div style={{ y: orb1Y }} className="absolute top-[8%] left-[3%] w-[550px] h-[550px] rounded-full bg-purple-500/[0.05] blur-[150px]" />
      <motion.div style={{ y: orb2Y }} className="absolute top-[55%] right-[0%] w-[450px] h-[450px] rounded-full bg-cyan-500/[0.04] blur-[130px]" />
          ...
          <div className="container-xl px-6">
            <motion.div style={{ y: heroY, opacity: heroOpacity }} className="max-w-5xl">
              <div className="flex items-center gap-3 mb-8">
                <motion.div initial={{ width: 0 }} animate={{ width: "48px" }} transition={{ duration: 0.8, delay: 0.2, ease: [0.22, 1, 0.36, 1] }} className="h-px bg-purple-500" />
                <motion.span initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.7 }} className="text-[10px] font-black uppercase tracking-[0.5em] text-purple-400">Automation System</motion.span>
              </div>
              ...
              >
                Eliminating manual administration with autonomous scalability engines.
              </motion.p>
        ...
        <section className="relative z-10 pb-20">
          <div className="container-xl px-6">
            <motion.div initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }} className="flex items-center gap-3 mb-12">
              <div className="h-px w-7 bg-purple-500" />
              <span className="text-[10px] font-black uppercase tracking-[0.5em] text-purple-400">Core Capabilities</span>
            </motion.div>
            ...
            <div className="grid lg:grid-cols-2 gap-20 items-center">
              <div className="space-y-4 order-2 lg:order-1">
                {features.map((item, i) => (
                  ...
                  >
                    <div className="w-12 h-12 rounded-2xl bg-white/[0.03] border border-white/10 flex items-center justify-center shrink-0 group-hover:scale-110 group-hover:border-purple-500/50 transition-all duration-500 shadow-2xl">
                    ...
                    <div className="absolute right-6 top-6 text-[8px] font-black text-white/5 group-hover:text-white/10 opacity-0 group-hover:opacity-100 uppercase tracking-widest transition-all">Auto_Node_0{i + 1}</div>
                  </motion.div>
                ))}
              </div>
              <ParallaxImagePanel />
            ...
            >
              ...
              <motion.div initial={{ opacity: 0, scale: 0.95 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} transition={{ delay: 0.4 }}>
                <Link
                  ...
                  className="px-12 py-5 rounded-full bg-white text-black font-black uppercase tracking-[0.2em] text-sm transition-all hover:bg-purple-600 hover:text-white hover:scale-105 active:scale-95 shadow-[0_0_40px_rgba(255,255,255,0.1)] inline-flex items-center gap-3"
                >
                  Configure Automation <ArrowRight size={18} />
                </Link>
              </motion.div>
            </motion.div>
          </div>
