...
/* ─── Data ──────────────────────────────────────────────────────── */
const features = [
  { title: "Actionable Insights", desc: "Converts raw user data into visual summaries and trend predictions with high precision." },
    ...
    <div ref={ref} className="relative aspect-square lg:order-2">
      ...
      >
        ...
        <motion.div
          ...
          className="absolute inset-x-0 h-1/2 bg-gradient-to-b from-cyan-400/0 via-cyan-400/10
...
function WorkflowSection() {
  ...
  const nodes = useMemo(() => [
    { cx: 220, cy: 75, num: 1, label: "Ingest Data",      desc: "Processes raw multi-tenant information in real-time as it flows through shards.", color: "#00F0FF", above: false },
    { cx: 450, cy: 25, num: 2, label: "Interpret",        desc: "AI engine sits on top of shards to translate data into actionable insights.",     color: "#9D5BFF", above: true  },
    { cx: 670, cy: 75, num: 3, label: "Analyze",          desc: "Identify popular features and predict user segments likely to churn.",           color: "#00F0FF", above: false },
    { cx: 880, cy: 25, num: 4, label: "Executive Dash",  desc: "Delivering high-level visual summaries for strategic decision-making.",           color: "#9D5BFF", above: true  },
  ], []);
...
function BgOrbs() {
  ...
  return (
    <div ref={ref} className="absolute inset-0 pointer-events-none overflow-hidden z-0">
      <motion.div style={{ y: orb1Y }} className="absolute top-[8%] left-[3%] w-[550px] h-[550px] rounded-full bg-cyan-500/[0.05] blur-[150px]" />
      <motion.div style={{ y: orb2Y }} className="absolute top-[55%] right-[0%] w-[450px] h-[450px] rounded-full bg-fuchsia-500/[0.04] blur-[130px]" />
      ...
      <main ref={pageRef} className="bg-[#020205] text-white relative overflow-hidden">
          ...
          <div className="container-xl px-6">
            <motion.div style={{ y: heroY, opacity: heroOpacity }} className="max-w-5xl">
              <div className="flex items-center gap-3 mb-8">
                <motion.div initial={{ width: 0 }} animate={{ width: "48px" }} transition={{ duration: 0.8, delay: 0.2, ease: [0.22, 1, 0.36, 1] }} className="h-px bg-cyan-400" />
                <motion.span initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.7 }} className="text-[10px] font-black uppercase tracking-[0.5em] text-cyan-400">AI Solutions</motion.span>
              </div>
              ...
              >
                Integrating AI as a core intelligence engine within multi-tenant systems.
              </motion.p>
              ...
              <div className="space-y-4 order-2 lg:order-1">
                {features.map((item, i) => (
                  ...
                  >
                    ...
                    <div className="absolute right-6 top-6 text-[8px] font-black text-white/5 group-hover:text-white/10 opacity-0 group-hover:opacity-100 uppercase tracking-widest transition-all">AI_Node_0{i + 1}</div>
                  </motion.div>
                ))}
              </div>
              <ParallaxImagePanel />
        ...
        <section className="relative z-10 py-32">
          <div className="max-w-4xl mx-auto px-6">
            ...
            >
              <motion.div animate={{ x: ["-100%", "200%"] }} transition={{ duration: 3.5, repeat: Infinity, ease: "linear" }} className="absolute top-0 left-0 w-72 h-[2px] bg-gradient-to-r from-transparent via-cyan-500 to-transparent" />
              ...
              <h2 className="text-3xl md:text-6xl lg:text-7xl font-black mb-8 leading-[0.9] tracking-tighter italic uppercase text-white/90">
                Future-proof <span className="gradient-text-primary">Ecosystem</span>
              </h2>
              ...
              <motion.div initial={{ opacity: 0, scale: 0.95 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} transition={{ delay: 0.4 }}>
                <Link
                  ...
                  className="px-12 py-5 rounded-full bg-white text-black font-black uppercase tracking-[0.2em] text-sm transition-all hover:bg-cyan-400 hover:text-black hover:scale-105 active:scale-95 shadow-[0_0_40px_rgba(255,255,255,0.1)] inline-flex items-center gap-3"
                >
                  Explore AI Workflows <ArrowRight size={18} />
                </Link>
              </motion.div>
            </motion.div>
          </div>
